-- Module-scoped style jobs: allow ai_jobs without a project and tag the owning
-- product module so feeds can filter (projects vs standalone Style workspace).
alter table public.ai_jobs
  alter column project_id drop not null;

alter table public.ai_jobs
  add column module text not null default 'projects'
    constraint ai_jobs_module_check check (module in ('projects', 'style'));

create index ai_jobs_module_style_idx on public.ai_jobs(module, (input->>'style_id'));

-- Standalone style jobs carry no project and always declare a style.
alter table public.ai_jobs
  add constraint ai_jobs_module_shape_check check (
    (module = 'projects' and project_id is not null)
    or (module = 'style' and project_id is null and input->>'style_id' is not null)
  );

-- Style-aware module scope for the existing job RPC: same signature prefix plus
-- two trailing defaults, so existing callers keep working unchanged.
...
revoke all on function public.enqueue_ai_job(uuid, uuid, uuid, text, text, text, text, integer, text, text, uuid, uuid, text, uuid, text, text) from public, anon;
grant execute on function public.enqueue_ai_job(uuid, uuid, uuid, text, text, text, text, integer, text, text, uuid, uuid, text, uuid, text, text) to authenticated;

-- Style module jobs are readable via the same workspace scoping the table RLS
-- already enforces (select-only for authenticated members).
