-- Replace Pollinations with Google AI Studio Nano Banana 2.
...
begin
  ...
  if p_model not in ('openai/gpt-image-2', 'google/gemini-3.1-flash-image')
     or (p_model = 'openai/gpt-image-2' and p_provider <> 'openai')
     or (p_model = 'google/gemini-3.1-flash-image' and (p_provider <> 'google' or p_operation <> 'text_to_image')) then
  ...
  if p_operation = 'text_to_image' then
    ...
    if p_size not in ('1024x1024', '1536x1024', '1024x1536', 'auto')
       or (p_provider = 'google' and p_size = 'auto') then
      raise exception 'INVALID_REQUEST' using errcode = '22023';
    end if;
  else
...
begin
  ...
  if v_job.status in ('succeeded', 'failed', 'canceled', 'persisting')
     or (v_job.provider in ('openai', 'google') and v_job.status in ('submitting', 'processing')) then
