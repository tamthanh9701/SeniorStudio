-- 0041_style_variant_source_link.sql
-- 0039 folded the source lookup into the inpaint branch only, so an
-- image_to_image style job was enqueued with a null source: the worker rejected
-- it ("image_to_image requires a source version") and completion would have
-- failed its lineage check even if the provider had been paid.
-- The lookup now covers both operations, and the resolved source is recorded on
-- the job (asset_id, parent_version_id and the source_version_id column the
-- worker reads).  0001-0040 remain immutable.
