-- 0045_library_reference_selection.sql
-- A generation may send references borrowed from another style of the same
-- library, recorded in the packet as metadata.library_reference_ids. The enqueue
-- function accepts such a reference only while it is live and its owning style
-- still shares the job style's library; everything else stays exactly as 0044
-- left it. 0001-0044 remain immutable.
