...
export default function EditAssetPage() {
  ...
  const inspector = <div className="p-4 pt-16 xl:pt-4"><div className="flex items-center gap-2"><Paintbrush className="size-4 text-primary" /><h1 className="font-semibold">Inpaint</h1></div><p className="mt-2 text-xs leading-5 text-muted-foreground">Transparent mask regions are edited against the exact current parent version.</p><div className="mt-6 space-y-4"><div className="space-y-2"><Label htmlFor="inpaint-model" className="text-xs font-semibold">Model</Label><Select value={modelId} onValueChange={(value) => { const nextModel = models.find((model) => model.id === value); if (!nextModel) return; setModelId(nextModel.id); setQuality(nextModel.qualities[0]); }}><SelectTrigger id="inpaint-model" className="w-full"><SelectValue /></SelectTrigger><SelectContent>{models.map((model) => <SelectItem key={model.id} value={model.id}>{model.label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="inpaint-quality" className="text-xs font-semibold">Quality</Label><Select value={quality} onValueChange={(value) => setQuality(value as SupportedQuality)}><SelectTrigger id="inpaint-quality" className="w-full"><SelectValue /></SelectTrigger><SelectContent>{selected.qualities.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="inpaint-prompt" className="text-xs font-semibold">Prompt</Label><Textarea id="inpaint-prompt" className="min-h-32 resize-none" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Describe the requested edit" /></div>{job && <div role="status" aria-live="polite" className="rounded-xl border border-white/10 bg-white/[0.025] p-3 text-sm text-muted-foreground">{JOB_STATUS_LABELS[job.status]}</div>}{error && <p role="alert" aria-live="assertive" className="text-sm text-destructive">{error}</p>}</div></div>;
