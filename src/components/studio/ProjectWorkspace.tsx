...
import { AiJobSchema, type ProjectJobFeedItem, type SupportedModelId } from "@/db/ai-jobs";
...
export default function ProjectWorkspace({ project, projects, userEmail, assets, models, initialJobs }: { project: { id: string; name: string }; projects: Array<{ id: string; name: string }>; userEmail: string; assets: WorkspaceAsset[]; models: ModelCatalogEntry[]; initialJobs: ProjectJobFeedItem[] }) {
  ...
  const resultAssets = items.flatMap(({ job, result_urls }) => result_urls.map((url, index) => { const results = Array.isArray(job.output.results) ? job.output.results as Array<{ asset_id?: string; version_id?: string }> : []; return { id: results[index]?.asset_id ?? `${job.id}-${index}`, name: job.input.prompt, signedUrl: url, versionId: results[index]?.version_id ?? null, createdAt: job.created_at }; }));
  ...
  const retry = (job: ProjectJobFeedItem["job"]) => { setPrompt(job.input.prompt); setSettings({ modelId: job.model, size: job.input.size, quality: job.input.quality, count: job.input.count }); setTool("generate"); setFocusSignal((value) => value + 1); };
