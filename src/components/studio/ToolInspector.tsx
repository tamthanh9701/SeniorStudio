...
export default function ToolInspector({ tool, setTool, models, settings, setSettings, selectedAsset, projectId }: {
  ...
  return <div className="min-h-full p-4 pt-16 xl:pt-4">
    <div className="grid grid-cols-2 rounded-xl bg-white/[0.045] p-1"><button onClick={() => setTool("generate")} className={`flex min-h-10 items-center justify-center gap-2 rounded-lg text-sm font-medium ${tool === "generate" ? "bg-[var(--panel)] text-white shadow" : "text-[var(--muted)]"}`}><Sparkles className="size-4" />Generate</button><button onClick={() => setTool("inpaint")} className={`flex min-h-10 items-center justify-center gap-2 rounded-lg text-sm font-medium ${tool === "inpaint" ? "bg-[var(--panel)] text-white shadow" : "text-[var(--muted)]"}`}><Paintbrush className="size-4" />Inpaint</button></div>
