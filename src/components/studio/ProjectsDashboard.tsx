...
export default function ProjectsDashboard({ projects, userEmail }: { projects: DashboardProject[]; userEmail: string }) {
  ...
  const createProject = async (event: React.FormEvent) => {
    ...
    } catch (caught) { setCreateError(caught instanceof Error ? caught.message : "Unable to create project"); }
    finally { setSubmitting(false); }
  };

  const confirmDeleteProject = async () => {
    if (!deleteTarget || deletingProject) return;
    setDeletingProject(true); setDeleteError(null);
    try {
    ...
    } catch (caught) { setDeleteError(caught instanceof Error ? caught.message : "Unable to delete project"); }
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!next) setOpen(false); }}>
      ...
      >
        ...
        <form onSubmit={createProject} className="space-y-4">
          ...
          {createError && <Alert variant="destructive" role="alert" className="px-3 py-2"><AlertDescription className="text-xs">{createError}</AlertDescription></Alert>}
    ...
    <Dialog open={Boolean(deleteTarget)} onOpenChange={(next) => { if (!next && !deletingProject) setDeleteTarget(null); }}>
      ...
      >
        ...
        {deleteError && <Alert variant="destructive" role="alert" className="px-3 py-2"><AlertDescription className="text-xs">{deleteError}</AlertDescription></Alert>}
