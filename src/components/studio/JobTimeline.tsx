  ...
  return <div className="mx-auto w-full max-w-3xl space-y-8 px-4 py-8 sm:px-6">
    {items.map(({ job, result_urls }) => {
      ...
      return <article key={job.id} className="space-y-4">
        <div className="ml-auto max-w-[88%] rounded-2xl rounded-br-md bg-[var(--accent)] px-4 py-3 text-sm leading-6 text-white shadow-lg shadow-blue-500/10"><p className="whitespace-pre-wrap">{job.input.original_prompt ?? job.input.prompt}</p><time className="mt-2 block text-[11px] text-white/65">{new Date(job.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time></div>
