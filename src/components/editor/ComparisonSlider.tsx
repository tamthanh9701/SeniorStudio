...
export default function ComparisonSlider({ beforeUrl, afterUrl, width, height, beforeLabel = "Parent", afterLabel = "Current" }: { beforeUrl: string; afterUrl: string; width: number; height: number; /** Label for the left (before) image. */ beforeLabel?: string; /** Label for the right (after) image. */ afterLabel?: string }) {
  ...
  return <div ref={containerRef} className="relative mx-auto w-full max-w-full touch-none overflow-hidden rounded-2xl border border-white/10 bg-black" style={{ aspectRatio: `${width} / ${height}` }} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); update(event.clientX); }} onPointerMove={(event) => { if (event.currentTarget.hasPointerCapture(event.pointerId)) update(event.clientX); }}>
    <img src={afterUrl} alt={`${afterLabel} version`} className="absolute inset-0 h-full w-full object-contain" draggable={false} />
    <img src={beforeUrl} alt={`${beforeLabel} version`} className="absolute inset-0 h-full w-full object-contain" style={{ clipPath: `inset(0 ${100 - position}% 0 0)` }} draggable={false} />
    ...
    <span className="absolute left-3 top-3 rounded-lg bg-black/70 px-2 py-1 text-xs font-medium text-[var(--stage-text)]">{beforeLabel}</span><span className="absolute right-3 top-3 rounded-lg bg-black/70 px-2 py-1 text-xs font-medium text-[var(--stage-text)]">{afterLabel}</span>
  </div>;
}
